...
(function() {
  ...
  oj.addMethods = function(obj, mapNameToMethod) {
    ...
    for (methodName in mapNameToMethod) {
      ...
      _results.push(oj.addMethod(obj, methodName, method));
    }
    return _results;
  };
  ...
  oj.addProperties = function(obj, mapNameToInfo) {
    var propInfo, propName, _results;
    _results = [];
    for (propName in mapNameToInfo) {
      propInfo = mapNameToInfo[propName];
      if (!(_.isObject(propInfo)) || !(propInfo.get != null)) {
        propInfo = {
          value: propInfo
        };
      }
      _results.push(oj.addProperty(obj, propName, propInfo));
    }
    return _results;
  };
  ...
  */
  ...
  oj.createType = function(name, args) {
    var Out, methodKeys, propKeys, _ref1, _ref2;
    if (args == null) {
      args = {};
    }
    if (!_.isString(name)) {
    ...
    if (Out.prototype.properties != null) {
      Out.prototype.properties = _.uniqueSortedUnion(Out.prototype.properties, propKeys);
    } else {
    ...
    if (Out.prototype.methods != null) {
      Out.prototype.methods = _.uniqueSortedUnion(Out.prototype.methods, methodKeys);
    } else {
  ...
  oj.createView = function(name, args) {
    var _ref1;
    if (!_.isString(name)) {
      throw 'oj.createView: string expected for first argument';
    }
    if (!_.isObject(args)) {
      throw 'oj.createView: object expected for second argument';
    }
    if ((_ref1 = args["extends"]) == null) {
      args["extends"] = oj.View;
    }
    return oj.createType(name, args);
  };
