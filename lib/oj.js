...
(function() {
  var ArrayP, FuncP, ObjP, coffee, compileJS, concat, dhp, doctypes, exports, jqName, jqueryEvents, key, oj, ojName, pass, pathNormalizeArray, pathSplit, pathSplitRe, plugins, root, slice, strict4, strict5, stripBOM, t, unshift, w3, wrapCSMessage, wrapJS, wrapJSMessage, _, _argumentsStack, _attributeCMeansClassAndAllowsArrays, _attributeClassAllowsArrays, _attributeOmitFalsyValues, _attributeStyleAllowsObject, _attributesBindEventsToDOM, _attributesFilterOutEvents, _attributesFromObject, _attributesProcessedForOJ, _boundOrThrow, _breaker, _chars, _clone, _compileAny, _compileDeeper, _compileTag, _contains, _cssFromObject, _dasherize, _decamelize, _defaults, _each, _extend, _flatten, _flattenCSSMap, _flattenCSSMap_, _fn, _fn1, _getInstanceOnElement, _getTagName, _has, _i, _identity, _indexOf, _inherit, _isCapitalLetter, _isEmpty, _j, _jqGetValue, _jqueryExtend, _keys, _len, _len1, _omit, _randomInteger, _readyOrLoad, _readyQueue, _reduce, _ref, _ref1, _ref2, _setInstanceOnElement, _setObject, _setTagName, _some, _sortedIndex, _splitAndTrim, _styleFromObject, _toArray, _uniqueSort, _uniqueSortedUnion, _values,
    __slice = [].slice;
  ...
  oj.View.css = function(def) {
    oj.View.styles.push(_setObject({}, ".oj-" + this.typeName, def));
  };

  oj.View.theme = function(name, def) {
    def = _setObject({}, ".oj-" + this.typeName + ".theme-" + (_dasherize(name)), def);
    this.type.themes.push(_setObject({}, _dasherize(name), def));
    this.type.styles.push(def);
  };
