...
(function() {
  var ArrayP, FuncP, ObjP, coffee, concat, exports, fs, jqueryEvents, jqueryExtendEachMap, key, oj, pass, pathNormalizeArray, pathSplit, pathSplitRe, root, slice, stripBOM, t, unshift, wrapCSMessage, wrapJS, wrapJSMessage, _, _attributeCMeansClass, _attributeClassAllowsArrays, _attributeStyleAllowsObject, _attributesBindEventsToDOM, _attributesFilterOutEvents, _attributesFromObject, _attributesProcessedForOJ, _chars, _compileAny, _compileDeeper, _compileTag, _cssFromObject, _defaultClear, _fn, _i, _j, _len, _len1, _randomInteger, _readyOrLoad, _readyQueue, _ref, _ref1, _styleFromObject, _styleKeyFromFancy, _tagAttributes,
    __slice = [].slice;
  ...
  _compileTag = function(ojml, options) {
    ...
    tag = ojml[0];
    tagType = typeof tag;
    tag = (tagType === 'function' || tagType === 'object') && (tag.typeName != null) ? tag.typeName : tag;
  ...
  oj.View = oj.type('View', {
    constructor: function(args) {
      ...
      if ((_ref1 = args.id) == null) {
        args.id = oj.id();
      }
      this.$el.addClass(this.typeName);
      this.set(args);
      args = _.omit.apply(_, [args].concat(__slice.call(this.properties)));
      return this.addAttributes(args);
    },
    properties: {
      el: null,
      $el: {
  ...
  oj.ModelsView = oj.type('ModelsView', {
    ...
    constructor: function(args) {
      ...
      this.each = oj.argumentShift(args, 'each');
      this.models = oj.argumentShift(args, 'models');
    ...
    properties: {
      ...
      models: {
        ...
        set: function(v) {
          ...
          this._models = v;
          if (oj.isBackbone(this._models)) {
            this._models.on('add', this.modelAdd, this);
            this._models.on('remove', this.modelRemove, this);
            this._models.on('change', this.modelChange, this);
            this._models.on('sync', this.modelSync, this);
          }
          this.modelsChange();
        }
      }
    ...
    methods: {
      ...
      modelsChange: function() {
        return this.make(this.models);
      },
      modelAdd: function(model, collection, options) {
        console.log("ModelsView.modelAdd: ", arguments);
        return this.add(model, options.index);
      },
      modelRemove: function(model, collection, options) {
        console.log("ModelsView.modelRemove: ", arguments);
        return this.remove(model, options.index);
      },
      modelChange: function(model, collection, options) {
        return console.log("ModelsView.modelChange: ", arguments);
      },
      modelSync: function(model, collection, options) {
  ...
  oj.List = oj.type('List', {
    ...
    methods: {
      make: function(models) {
        var _this = this;
        if (!((this._models != null) && (this._each != null))) {
          return;
        }
        ...
        return this.$el.oj(function() {
          ...
          for (_j = 0, _len1 = models.length; _j < _len1; _j++) {
            model = models[_j];
            _results.push(_this.each(model, oj.li));
          }
          return _results;
        });
      },
      item: function(ix) {
        return this.$('> li').get(ix);
      },
      $item: function(ix) {
        return $(this.$('> li').get(ix));
      },
      addItem: function(ix, ojml) {
        if (ix == null) {
          ix = this.count - 1;
        }
      },
      removeItem: function(ix, ojml) {
        if (ix == null) {
          ix = this.count - 1;
        }
      },
      moveItem: function(ixFrom, ixTo) {
        var ojml;
        ojml = this.removeItem(ixFrom);
  ...
  oj.ModelView = oj.type('ModelView', {
    base: oj.View,
    constructor: function(args) {
      this.value = oj.argumentShift(args, 'value');
      this.model = oj.argumentShift(args, 'model');
    ...
    properties: {
      model: {
        ...
        set: function(v) {
          ...
          this._model = v;
          if (oj.isBackbone(this._model)) {
            this._model.on('change', this.modelChange, this);
          }
          this.modelChange();
        }
      }
  ...
  oj.TextBox = oj.type('TextBox', {
    ...
    constructor: function(args) {
      ...
      return oj.TextBox.base.constructor.apply(this, arguments);
    },
    properties: {
      value: {
        get: function() {
  ...
  oj.CheckBox = oj.type('CheckBox', {
    ...
    constructor: function(args) {
      ...
      return oj.CheckBox.base.constructor.apply(this, arguments);
    },
    properties: {
      value: {
        get: function() {
  ...
  oj.TextArea = oj.type('TextArea', {
    ...
    constructor: function(args) {
      ...
      return oj.TextArea.base.constructor.apply(this, arguments);
    },
    properties: {
      value: {
        get: function() {
  ...
  oj.ListBox = oj.type('ListBox', {
    ...
    constructor: function(args) {
      ...
      this.options = oj.argumentShift(args, 'options');
      return oj.ListBox.base.constructor.apply(this, arguments);
    },
    properties: {
      value: {
        get: function() {
      ...
      options: {
        ...
        set: function(v) {
          if (!oj.isArray(v)) {
            throw new Error('oj.ListBox::options is not an array');
          }
          this._options = v;
          this.$el.oj(function() {
            var op, _j, _len1;
  ...
  jqueryExtendEachMap = function(cbEach, cbMap, isMap) {
    if (isMap == null) {
      isMap = 0;
    }
    return function() {
      var $el, args, arr, i, out, rvalue, _k, _l, _ref2, _ref3;
      args = _.toArray(arguments);
      $el = $(this);
      if (isMap && cbMap && args.length === 0) {
        arr = [];
        for (i = _k = 0, _ref2 = $el.length - 1; 0 <= _ref2 ? _k < _ref2 : _k > _ref2; i = 0 <= _ref2 ? ++_k : --_k) {
          out = cbMap($($el[i]));
          if (out != null) {
            arr.push(out);
          }
        ...
        out = $el;
        for (i = _l = 0, _ref3 = $el.length; 0 <= _ref3 ? _l < _ref3 : _l > _ref3; i = 0 <= _ref3 ? ++_l : --_l) {
  ...
  $.fn.oj = jqueryExtendEachMap((function($el, args) {
    var d, dom, ojml, _k, _len2;
    if (args.length === 0) {
      return $el[0].oj;
    }
