...
(function() {
  var basenameForExtensions, coffee, commonPath, compileDir, compileFile, compilePath, csso, error, fullPaths, isAppModule, isDirectory, isFile, isHiddenFile, isNodeModule, isOJFile, isOJPage, isRelativeModule, isUnsupportedNodeModule, isWatchFile, isWatched, jsdom, ls, lsOJ, lsWatch, m, minifyCSS, minifyCSSUnless, minifyJS, minifyJSUnless, minifySimpleJS, minifySimpleJSUnless, mkdirp, nodeModulePaths, nodeModulesLinkMap, oj, pass, readFileSync, relativePathWithEscaping, resolveLink, spaces, success, tabs, triggerWatched, trimArgList, uglifyjs, unwatchAll, unwatchDir, verbose, verbosity, wait, watchCache, watchDir, watchFile, watchParents, _, _buildFileCache, _buildNativeCache, _buildNativeCacheFromModuleList, _buildRequireCache, _clearRequireCacheRecord, _createHook, _escapeSingleQuotes, _first, _getRequiresInSource, _hookRequire, _i, _insertAt, _len, _length, _minifyAndWrapCSSInStyleTags, _nativeModuleCode, _nodeModuleUnsupported, _nodeModulesSupported, _ojModuleCode, _ref, _rememberModule, _rememberModuleDependencies, _requireCache, _requireCacheToString, _restoreRequireCache, _saveRequireCache, _startsWith, _trim, _unhookRequire;
  ...
  nodeModulesLinkMap = function(fileDir) {
    var dir, dirs, linkPath, moduleName, modulePath, modules, out, _j, _k, _len1, _len2;
  ...
  compileFile = function(filePath, includeDir, options, cb) {
    var cache, cacheLength, css, deltaTime, dirOut, fileBaseName, fileDir, fileOut, hookCache, hookOriginalCache, html, includedModules, isDebug, moduleLinkMap, moduleParents, modules, ojml, outputDir, results, rootDir, scriptHtml, scriptIndex, startTime, styleHtml, styleIndex, subDir, timeStamp, _j, _len1;
    if (options == null) {
      options = {};
    }
    if (cb == null) {
  ...
  watchFile = function(filePath, includeDir, options) {
    var compileTimeout, prevStats, timeEpsilon, timeLast, watcher, _onWatch, _rewatch, _unwatch, _watchErr;
    if (options == null) {
      options = {};
    }
  ...
  _minifyAndWrapCSSInStyleTags = function(css, filePath, isDebug, structureOff) {
    var css_, newline;
    css_ = minifyCSSUnless(isDebug, filePath, css, structureOff);
    newline = isDebug ? '\n' : '';
    return "<style>" + newline + css_ + newline + "</style>";
  };
