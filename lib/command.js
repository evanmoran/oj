...
(function() {
  var coffee, commander, error, fs, isAppModule, isDirectory, isFile, isNodeModule, isRelativeModule, isUnsupportedNodeModule, minifyCSS, minifyJS, nodeModuleUnsupported, nodeModulesSupported, oj, path, success, trimArgList, uglifyjs, vm, watch, _, _buildCache, _cache, _cacheToString, _clearRequireCache, _first, _getModuleRequires, _nativeModuleCode, _replaceRequire;
  ...
  module.exports = function() {
    var cacheString, crypto, dirType, dirTypeOption, filePath, ix, stripBOM, _cacheHooks, _cacheOrigs, _commonPath, _createHook, _hookRequire, _i, _j, _len, _len1, _ref, _ref1, _remember, _remembered;
    commander.version('0.0.5').usage('[options] <file, ...>').option('-o, --output <dir>', 'Directory to output all files to (default: .)', process.cwd()).option('-d, --debug', 'Turn on debug output (default: false)', false).option('-r, --recurse', 'Recurse into directories (default: off)', false).option('-w, --watch <dir,dir,...>', 'Directories to watch (default: off)', trimArgList).option('-v, --verbose', 'Turn on verbose output (default: off)').parse(process.argv);
  ...
  _replaceRequire = function(code, prev, next) {
    var r;
    prev = prev.replace(/\\/g, '\\\\');
    prev = prev.replace(/\./g, '\\.');
  ...
  _getModuleRequires = function(code) {
    var match, out, r;
