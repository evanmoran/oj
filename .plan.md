...
### Bugs
  ...
  * Bug(P1): Twitter follow button doesn't work first time in try editor
  * Bug(P1): Try editor should clear display when code is cleared
  * Bug(P2): Try editor does not update createType CSS when changed in editor
  * Bug(P3): Try editor highlights "var", "getFullYear", "getMonth", etc with orange instead of yellow gold (or nothing for methods)
  * Bug(P3): Try editor doesn't highlight "constructor", "get", or "set" as orange
...
### Misc
  ...
  * Bug: Prevent try editor double compile (abandon html tags?)

  * Bug: Full screen button should be hidden on try editor when resized for phone

  * Bug: Literal notation should not require object at start. Use argumentsUnion

  * Bug: var tb = new TextBox('Batman');  // Should not emit
