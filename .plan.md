...
### Website
  ...
  *  Github button plugin derived from twitterFollowButton syntax (showCount:true)

### Misc

  * Consider using `with` keyword on oj jquery plugins

  * Bug: Literal notation should not require object at start. Use argumentsUnion
...
CSS
-------------------------------------------------------------------------------
How to generate CSS server and client side. Server side so it renderes immediately. Client side so it can dynamically work.

oj.type 'Table',
  css:

### Solution 1: <style> tags + object css seperation

#### OJ

0. Change compile of View to check css property and correct append css
1. Change css methods to have extra indirection from css source
2. Change toCSS to handle this indirection
3. Change compile to handle this indirection
  a. Add compile option to compile to styles tags (instead of css)
  b. Unit test the oj.compile method to do these things

#### Server Side
1. When styles are generated server side put the result in <style> tags marked by source
2. Change loading code to not generate global css when style c:'oj' is present

#### Client Side

1. When css is generated where it comes from is stored
2. When ojml is inserted through jquery plugins css is added with <style> tags (appended inside of <head>) unless it exists.
3. When the page first loads a great deal of global css is generated unless it exists

Plugin Rewriting

1. All css should be built into the type as css (read only property)
2. Rewrite to use css property
3. Add if statements to allow including without node

Include without Node
...
1.  Include all scripts and links included by main page
2.  Let the page render itself from within itself

  1 When css is built up record if it is global ('oj') or comes from an object ('oj')
  2 Server side output as script tags.
  3 Object css is generated client side when not present in script tags
  4 Global css is generated client side when not present in the script tag

  * In this way when changing Page (new feature) global css would be regenerated, but object CSS would remain.
  * When using new objects css would added as needed
  * Debuging should work because tags are being manipulated directly
