...
(function() {
  var basenameForExtensions, coffee, commonPath, compileDir, compileFile, compileJS, compilePath, csso, error, fullPaths, isAppModule, isDirectory, isFile, isHiddenFile, isNodeModule, isOJDir, isOJFile, isOJPage, isRelativeModule, isUnsupportedNodeModule, isWatchFile, isWatched, jsdom, ls, lsOJ, lsWatch, m, mkdirp, nodeModulePaths, nodeModulesLinkMap, oj, pass, readFileSync, relativePathWithEscaping, renderPath, resolveLink, spaces, stripBOM, success, tabs, triggerWatched, trimArgList, uglifyjs, unwatchAll, unwatchDir, verbose, verbosity, wait, watchCache, watchDir, watchFile, watchParents, wrapCSMessage, wrapJS, wrapJSMessage, _, _buildFileCache, _buildNativeCache, _buildNativeCacheFromModuleList, _buildRequireCache, _clearRequireCacheRecord, _createHook, _escapeSingleQuotes, _first, _getRequiresInSource, _hookRequire, _i, _insertAt, _len, _length, _nativeModuleCode, _nodeModuleUnsupported, _nodeModulesSupported, _ojModuleCode, _outputCss, _outputDataToFileOrCallback, _outputFile, _outputHtml, _outputModules, _outputPages, _outputUnifiedHtml, _ref, _rememberModule, _rememberModuleDependencies, _requireCache, _requireCacheToHtml, _requireCacheToJS, _restoreRequireCache, _saveRequireCache, _startsWith, _timeStampFromStartTime, _trim, _unhookRequire;
  ...
  oj.command = function(options) {
    var fullPath, _j, _len1, _ref1, _ref10, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
    ...
    if ((_ref4 = options.include) == null) {
      options.include = [];
    }
    if ((_ref5 = options.exclude) == null) {
      options.exclude = [];
    }
    options.output = (path.resolve(process.cwd(), (_ref6 = options.output) != null ? _ref6 : www)) + '/';
    options.modulesDir = (path.resolve(process.cwd(), (_ref7 = options.modulesDir) != null ? _ref7 : './modules')) + '/';
    if ((_ref8 = options.cssDir) == null) {
      options.cssDir = (path.resolve(process.cwd(), (_ref9 = options.cssDir) != null ? _ref9 : './styles')) + '/';
    }
  ...
  compileFile = function(filePath, includeDir, options, cb) {
    var cache, compileOptions, cssMapOption, cssOption, eCompile, eRequire, ex, extOut, fileBaseName, fileDir, fileOut, hookCache, hookOriginalCache, htmlOption, includedModules, isMinify, moduleLinkMap, moduleParents, modules, ojml, outputDir, results, rootDir, startTime, subDir, _j, _k, _len1, _len2, _ref1, _ref2, _ref3;
    ...
    cssOption = options.onlyCss ? true : false;
    cssMapOption = options.onlyHtml || options.onlyModules ? false : true;
    htmlOption = options.onlyCss || options.onlyModules ? false : true;
    compileOptions = {
      minify: isMinify,
      html: htmlOption,
      cssMap: cssMapOption,
    ...
    extOut = options.onlyCss ? '.css' : options.onlyModules ? '.js' : '.html';
  ...
  _outputFile = function(options, cb) {
    if (options.onlyHtml) {
      ...
      return _outputPages(options, cb);
    } else if (options.onlyModules) {
      return _outputModules(options, cb);
    } else {
      return _outputUnifiedHtml(options, cb);
    }
  };

  _outputUnifiedHtml = function(options, cb) {
  ...
  _outputModules = function(options, cb) {
    var info, js;
  ...
  _outputPages = function(options, cb) {};
  ...
  renderPath = function(path, options, cb) {
  ...
  lsWatch = function(paths, options, cb) {
    ...
    lsOptions = _.extend({}, {
      recurse: options.recurse,
      filterFile: (function(basename, path) {
        return isWatchFile(basename);
      }),
      filterDir: function(basename, path) {
        return !_startsWith(path, options.output) && isOJDir(basename);
      }
    });
  ...
  ls = function(paths, options, cb) {
    ...
    if ((_ref3 = options.filterFile) == null) {
      options.filterFile = function() {
        return true;
      };
    }
    if ((_ref4 = options.filterDir) == null) {
  ...
  _requireCacheToJS = function(cache, filePath, isMinify, options) {
    var clientDir, clientFile, code, commonDir, js, moduleDir, moduleName, nameToMain, newline, _fileToString, _files, _find, _modules, _modulesToString, _native, _nativeModuleToString, _ref1, _ref2, _ref3, _run;
    ...
    _fileToString = function(filePath, code) {
      ...
      return "F['" + filePath + "'] = (function(module,exports){(function(require,process,global,__dirname,__filename){" + newline + code + "})(RR('" + filePath + "'),P,G,'" + fileDir + "','" + fileName + "');});" + newline + "\n";
    };
    _modules = "";
    _ref1 = cache.modules;
    ...
    if (!options.onlyCss && !options.onlyHtml && !options.onlyPages) {
